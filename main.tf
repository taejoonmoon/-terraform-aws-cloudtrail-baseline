...
resource "aws_sns_topic_policy" "local_account_cloudtrail" {
  ...
  policy = data.aws_iam_policy_document.cloudtrail_sns_policy[0].json
}
...
resource "aws_cloudtrail" "global" {
  ...
  sns_topic_name                = var.cloudtrail_sns_topic_enabled ? aws_sns_topic.cloudtrail_sns_topic[0].arn : null
